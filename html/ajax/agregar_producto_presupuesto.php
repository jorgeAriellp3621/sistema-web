...
<table class="table table-bordered table-striped table-hover">
    ...
    <?php 
        ...
        while($row = mysqli_fetch_array($sql)){
            ...
            $precio_venta_f = number_format($precio_venta_ ?? 0); // Formatear a número si es null
            $precio_venta_r = str_replace(",", "", $precio_venta_f); // Reemplazar la coma
            $precio_total = $precio_venta_r * $cantidad;
            $precio_total_f = number_format($precio_total ?? 0); // Formatear a número si es null
    ...
    <span class="pull-right">
        <a href="#" onclick="eliminar('<?php echo $id_tmp; ?>')">
            <i class="fas fa-trash"></i> <!-- Ícono de papelera de Font Awesome -->
        </a>
    </span>
</td>
            </tr>
